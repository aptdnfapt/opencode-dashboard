...
export function TokenFlowChart({ data, height = 200 }: TokenFlowChartProps) {
  ...
  return (
    <div ref={containerRef} className="relative w-full">
      <svg ref={svgRef} />
      <div className="flex gap-2 sm:gap-3 mt-2.5 sm:mt-3 pt-2.5 sm:pt-3 border-t border-white/10">
        <div className="flex items-center gap-1.5 sm:gap-2">
          <div className="w-2.5 h-0.5 sm:w-3 rounded bg-[#635bff]" />
          <span className="text-[10px] sm:text-xs text-gray-400">Input</span>
        </div>
        <div className="flex items-center gap-1.5 sm:gap-2">
          <div className="w-2.5 h-0.5 sm:w-3 rounded bg-[#10b981]" />
          <span className="text-[10px] sm:text-xs text-gray-400">Output</span>
        </div>
      </div>
      {tooltip && (
        <div
